...
This lets the weekly job commit the refreshed HTML and `data/prices.csv`. Without it the
...
The build publishes `index.html` to the repository root, so root serving works with no
folder juggling. A `.nojekyll` file is written alongside it, which stops GitHub from
rendering `README.md` as the homepage instead of the dashboard.
...
3. rebuilds both dashboards at the repo root,
...
index.html                   published landing page  (GitHub Pages serves the root)
speculative.html             published dashboard
managed-money.html           published dashboard
.nojekyll                    disables Jekyll so index.html wins over README.md
```
...
| Pages 404 | Settings → Pages must point at branch `main`, folder `/ (root)`; check `index.html` is committed |
| Page shows the README instead of the dashboard | `index.html` and `.nojekyll` missing from the root — run `python build.py`, then `git add -f index.html .nojekyll` and push |
