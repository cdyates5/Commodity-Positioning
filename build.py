...
self-contained HTML dashboards into docs/.
  ...
  managed-money.html   Managed money net as % of MM gross (longs + shorts)   [Disaggregated report, 2006+]
  speculative.html     Non-commercial net as % of total open interest        [Legacy report, 2000+]
  index.html           Landing page linking both

Output goes to the REPOSITORY ROOT so GitHub Pages can serve it with
...
# Published output location. ROOT = repo root (Pages: main -> / (root)).
# Change to `ROOT / "docs"` if you prefer Pages served from the /docs folder.
OUT = ROOT
DATA = ROOT / "data"
...
def main() -> int:
    ...
    (OUT / ".nojekyll").write_text("")   # stop Jekyll rendering README.md as the homepage
